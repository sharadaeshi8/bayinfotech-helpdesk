...
const AnalyticsDashboard = () => {
  ...
  const [realSystemPerf, setRealSystemPerf] = React.useState(systemPerformance);
  const [realCategories, setRealCategories] = React.useState(topIssueCategories);
    ...
    const loadMetrics = async () => {
      try {
        ...
        if (data.system_uptime !== undefined) {
            const uptimeDays = (data.system_uptime / 86400).toFixed(1);
            setRealSystemPerf(prev => ({
                ...prev,
                systemUptime: { ...prev.systemUptime, value: 99.9, period: `last ${uptimeDays} days` }, // Mock high uptime for demo
                activeConcurrentUsers: {
                    ...prev.activeConcurrentUsers,
                    value: data.active_users || 0,
                    percentage: Math.min(100, Math.round(((data.active_users || 0) / 1000) * 100))
                }
            }));
        }
        ...
        if (data.tier_distribution) {
            ...
            if (total > 0) {
                const categories = [
                    { category: 'Access & Auth (Tier 1)', count: data.tier_distribution['TIER_1'] || 0 },
                    { category: 'Infrastructure (Tier 2)', count: data.tier_distribution['TIER_2'] || 0 },
                    { category: 'Critical/Security (Tier 3)', count: data.tier_distribution['TIER_3'] || 0 },
                ].map(c => ({
                    ...c,
  ...
  React.useEffect(() => {
    const loadTrends = async () => {
      try {
        ...
        // Transform backend data (hourly ISO) to chart format
        // Backend returns: { tickets: [{ time: "2023-11-27T10:00:00", count: 5 }, ...] }
        if (data && data.tickets) {
          const formattedTrends = data.tickets.map(t => ({
            time: new Date(t.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            count: t.count
          }));
          // Sort by time if needed, but backend usually returns sorted
    ...
    const interval = setInterval(loadTrends, 60000);
    return () => clearInterval(interval);
  }, []);
  ...
  const barChartData = {
    labels: realCategories.map(c => c.category),
    datasets: [
      {
        label: 'Ticket Count',
        data: realCategories.map(c => c.count),
        backgroundColor: realCategories.map((c, idx) => {
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context) {
            const category = realCategories[context.dataIndex];
    ...
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, bgcolor: '#1a1a1a', minHeight: '100vh', width: '100%' }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4, width: '100%' }} data-testid="charts-grid">
        ...
        <Grid size={{ xs: 12, lg: 4 }} data-testid="chart-card">
          ...
          >
            <Typography variant="h3" sx={{ mb: 3, color: '#D4AF37', fontWeight: 'bold' }}>
              Ticket Volume Trend (Last 24 Hours)
            </Typography>
            <Box sx={{ height: '320px' }}>
          ...
          >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Typography variant="body2" sx={{ color: '#4A7C59', fontWeight: 'bold' }}>
                    {realSystemPerf.dashboardLoadTime.value} {realSystemPerf.dashboardLoadTime.unit}
                  </Typography>
                </Box>
                <LinearProgress
                  variant="determinate"
                  value={(realSystemPerf.dashboardLoadTime.value / realSystemPerf.dashboardLoadTime.threshold) * 100}
                  sx={{
                    height: '6px',
                    backgroundColor: '#333333',
              ...
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Typography variant="body2" sx={{ color: '#D4AF37', fontWeight: 'bold' }}>
                    {realSystemPerf.dataRefreshInterval.value} {realSystemPerf.dataRefreshInterval.unit}
                  </Typography>
                </Box>
              </Box>
              ...
              <Box>
                ...
                <LinearProgress
                  variant="determinate"
                  value={realSystemPerf.activeConcurrentUsers.percentage}
                  sx={{
                    height: '6px',
                    backgroundColor: '#333333',
                    '& .MuiLinearProgress-bar': {
                      backgroundColor: realSystemPerf.activeConcurrentUsers.percentage < realSystemPerf.activeConcurrentUsers.threshold ? '#4A7C59' : '#FF9500',
                    },
                  }}
                />
              </Box>
              ...
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="body2" sx={{ color: '#E0E0E0' }}>
                    System Uptime ({realSystemPerf.systemUptime.period})
                  </Typography>
                  <Typography variant="body2" sx={{ color: '#4A7C59', fontWeight: 'bold' }}>
                    {realSystemPerf.systemUptime.value}%
                  </Typography>
                </Box>
                <LinearProgress
                  variant="determinate"
                  value={realSystemPerf.systemUptime.value}
                  sx={{
                    height: '6px',
                    backgroundColor: '#333333',
                    '& .MuiLinearProgress-bar': {
                      backgroundColor: realSystemPerf.systemUptime.value >= realSystemPerf.systemUptime.threshold ? '#4A7C59' : '#FF9500',
                    },
                  }}
                />
              </Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="body2" sx={{ color: '#4A7C59', fontWeight: 'bold' }}>
                            {realSystemPerf.resolutionRate !== undefined ? realSystemPerf.resolutionRate : '74%'}
                        </Typography>
                      </Box>
                      <LinearProgress variant="determinate" value={realSystemPerf.resolutionRateValue || 74} sx={{ height: 6, backgroundColor: '#333333', '& .MuiLinearProgress-bar': { backgroundColor: '#4A7C59' } }} />
                </Box>
                <Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        <Typography variant="body2" sx={{ color: '#E0E0E0' }}>Resolution Rate</Typography>
