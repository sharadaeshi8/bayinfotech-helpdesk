...
const TierChip = styled(Chip)(({ theme }) => ({
  marginTop: '4px',
  fontSize: '10px',
  height: '20px',
...
const ChatMessage = ({ message, isUser = false, source, confidence, sentiment, type, agentName, agentTier, isTyping = false, guardrail, ...props }) => {
        ...
        <BubbleContent isUser={isUser} isAgent={!!agentName} isGuardrail={type === 'guardrail'}>
          ...
          {/* Source attribution for AI messages */}
            ...
            <Box sx={{ mt: 1, display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
              ...
              {props.tier && (
                <TierChip
                  label={props.tier.replace('_', ' ')}
                  size="small"
                />
              )}
              {props.severity && (
                <SeverityChip
                  label={props.severity.toUpperCase()}
